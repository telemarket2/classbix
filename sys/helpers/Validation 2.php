...
class Validation
{
	...
	protected $_error_prefix = '<p class="msg-error-line">';
	protected $_error_suffix = '</p>';
	protected $error_string = '';
	protected $success_string = '';
	...
	/**
	 * return instance of Validation object 
	 * 
	 * @return Validation 
	 */
	...
	/**
	 * return instance if Input object
	 * @return Input 
	 */
	...
	function set_fields($data = '', $field = '')
	{
		...
		foreach ($data as $key => $val)
		{
			...
			{
				$this->_field_data[$key]['label'] = $val;
			}
		}
	}
	 ...
	 * 
	 * updated by veppa
	 * 	removed duplicate controls and changed function name from set_rules to set_rules2
	 *
	 * @access	public
	 * @param	mixed
	 * @param	string
	 * @return	void
	 */
	public function set_rules2($field, $label = '', $rules = '')
	{
		...
		if (is_array($field))
		{
			...
			{
				// Here we go!
				$this->set_rules2($row['field'], $row['label'], $row['rules']);
			}
			return $this;
		}
	...
	/**
	 * echo formatted error to display at the top of page 
	 * 
	 * by veppa
	 * 
	 * @param type $class 
	...
	/**
	 * return formatted error to display at the top of page. se internal error and success array and cookie values as well.
	 * 
	 * by veppa
	 * 
	 * @param type $class
	...
	public function messages_dump($class = 'msg')
	{
		...
		if ($this->info_array)
		{
			...
			$r .= '<div class="' . $class . '-info">' . implode(' </br>', $this->info_array) . '</div>';
		}
		...
		if (Flash::get('info'))
		{
			$r .= '<div class="' . $class . '-info">' . Flash::get('info') . '</div>';
		}
	...
	public function run($group = '')
	{
		...
		foreach ($this->_field_data as $field => $row)
		{
			...
			{
				/*
				 * EDITED 22.05.2019 empty fields should apply intval or other manupulation dor data 
				 * This prevents database insert errors
				 */
	...
	protected function _execute($row, $rules, $postdata = NULL, $cycles = 0)
	{
		...
		if (is_null($postdata) AND $callback == FALSE)
		{
			if (in_array('isset', $rules, TRUE) OR in_array('required', $rules))
			{
				...
				$this->set_error($message, $row['field']);
			}
		...
		foreach ($rules As $rule)
		{
			...
			if ($callback === TRUE)
			{
				...
				{
					// use callback with given class
					list($rule_class, $rule_method) = explode('::', $rule);
					if (method_exists($rule_class, $rule_method))
					{
						// Run the function and grab the result						
						$result = call_user_func(array($rule_class, $rule_method), $postdata, $param);
						//$result = {$rule_class}::{$rule_method}($postdata, $param);
					}
					else
					{
						continue;
			...
			if ($result === FALSE)
			{
				...
				{
					$line = __('Unable to access an error message corresponding to your field name.');
				}
				else
				{
	 ...
	 * 
	 * updated by veppa
	 * 	removed CI and used frog Record
	 *
	 * @access	public
	 * @param	string
	 * @param	field
	...
	public function is_unique($str, $field)
	{
		...
		$result = Record::findByIdFrom($table, $str, $field);
	 ...
	 * @return bool
	 */
	public function valid_url($url, $absolute = true)
	{
		if ($absolute)
		{
			return preg_match("
		...
		{
			return preg_match("/^(?:[\w#!:\.\?\+=&@$'~*,;\/\(\)\[\]\-]|%[0-9a-f]{2})+$/i", $url) ? true : false;
		}
	}
	...
	{
		return $this->input()->valid_ip($ip, $which);
	}
	...
	{
		return $this->input()->strip_image_tags($str);
	}
	...
	{
		return $this->input()->xss_clean($str);
	}
	...
	public function set_error($message, $field = '')
	{
		...
		{
			// old style of error
			$this->{$field . '_error'} = $this->_error_prefix . $message . $this->_error_suffix;

			// new style of setting error to field
