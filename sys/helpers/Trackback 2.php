...
class Trackback
{
	...
	function auto_discovery($text)
	{
		...
		// use this instead of eregi_replace because ereg depricated in php 5.3
		$arr_replace = array(
			'www.' => 'http://www.',
			'http://http://' => 'http://',
			'"' => ' "',
	...
	{
		$timestamp = mktime(
				substr($datetime, 8, 2), substr($datetime, 10, 2), substr($datetime, 12, 2), substr($datetime, 4, 2), substr($datetime, 6, 2), substr($datetime, 0, 4)
		);
